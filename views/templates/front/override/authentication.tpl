...
* If you did not receive a copy of the licene and are unable to
...
*}
{capture name=path}
    {if !isset($email_create)}{l s='Autenticación' mod='mtsalegraapi'}{else}
		<a href="{$link->getPageLink('authentication', true)|escape:'html':'UTF-8'}" rel="nofollow"
		   title="{l s='Autenticación' mod='mtsalegraapi'}">{l s='Autenticación' mod='mtsalegraapi'}</a>
		<span class="navigation-pipe">{$navigationPipe}</span>{l s='Crea tu cuenta'}
    {/if}
{/capture}
<h1 class="page-heading">{if !isset($email_create)}{l s='Autenticación' mod='mtsalegraapi'}{else}{l s='Crea tu cuenta' mod='mtsalegraapi'}{/if}</h1>
...
{if !isset($email_create)}
	...
	<div class="alert alert-danger">
		{if {$authentification_error|@count} == 1}
			<p>{l s='Existe al menos un error' mod='mtsalegraapi'} :</p>
			{else}
			<p>{l s='There are %s errors' sprintf=[$account_error|@count] mod='mtsalegraapi'} :</p>
		{/if}
		<ol>
	...
	<div class="row">
			...
			<form action="{$link->getPageLink('authentication', true)|escape:'html':'UTF-8'}" method="post"
				...
				<div class="form_content clearfix">
					...
					<div class="form-group">
						<label for="email_create">{l s='Correo Electrónico' mod='mtsalegraapi'}</label>
					...
					<div class="submit">
						...
						<button class="btn btn-default button button-medium exclusive" type="submit" id="SubmitCreate"
							...
							<span>
                                ...
                                {l s='Crear una cuenta' mod='mtsalegraapi'}
							</span>
						</button>
						<input type="hidden" class="hidden" name="SubmitCreate"
							   value="{l s='Crear una cuenta' mod='mtsalegraapi'}"/>
					</div>
				</div>
			</form>
		</div>
		<div class="col-xs-12 col-sm-6">
			<form action="{$link->getPageLink('authentication', true)|escape:'html':'UTF-8'}" method="post"
				...
				<h3 class="page-subheading">{l s='¿Ya esta registrado?' mod='mtsalegraapi'}</h3>
				<div class="form_content clearfix">
					<div class="form-group">
						<label for="email">{l s='Correo Electrónico' mod='mtsalegraapi'}</label>
					...
					<div class="form-group">
						<label for="passwd">{l s='Contraseña' mod='mtsalegraapi'}</label>
					...
					<p class="lost_password form-group"><a href="{$link->getPageLink('password')|escape:'html':'UTF-8'}"
														   title="{l s='Recuperar su contraseña' mod='mtsalegraapi'}"
														   rel="nofollow">{l s='¿Olvidó su contraseña?' mod='mtsalegraapi'}</a>
					</p>
					<p class="submit">
                        {if isset($back)}<input type="hidden" class="hidden" name="back"
												value="{$back|escape:'html':'UTF-8'}" />{/if}
